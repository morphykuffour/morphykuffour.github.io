...
double up the day the history catches up with it.
{%- endcomment -%}
{%- assign seen = "" -%}
<ul class="video-grid">
  {%- for track in tracks -%}
  ...
  <li class="video-item">
    <a class="video-cover" href="{{ track.link }}" rel="noopener">
      <img src="{{ site.baseurl }}/images/music/{{ track.file }}"
           alt="Cover for {{ track.title | escape }}"
           width="1280" height="720" loading="lazy" decoding="async">
      {%- if track.duration -%}<span class="video-time">{{ track.duration }}</span>{%- endif -%}
    </a>
    <a class="video-title" href="{{ track.link }}" rel="noopener">{{ track.title }}</a>
    {%- if track.channel != '' -%}
    <a class="video-channel" href="{{ track.channel_url }}" rel="noopener">{{ track.channel }}</a>
    {%- endif -%}
  </li>
  {%- endunless -%}
  {%- endfor -%}
